...
CREATE POLICY "own withdrawal insert" ON public.withdrawals FOR INSERT TO authenticated WITH CHECK (user_id = auth.uid() AND status = 'pending'::withdrawal_status AND amount > 0);
...
CREATE POLICY "agents self update" ON public.agents FOR UPDATE TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY "agent_wd self insert" ON public.agent_withdrawals FOR INSERT TO authenticated WITH CHECK (auth.uid() = agent_id AND status = 'pending' AND amount > 0);
...
END;
$$;
CREATE TRIGGER trg_protect_agent_columns BEFORE UPDATE ON public.agents FOR EACH ROW EXECUTE FUNCTION public.protect_agent_columns();
...
END;
$$;
CREATE TRIGGER trg_protect_profile_columns BEFORE UPDATE ON public.profiles FOR EACH ROW EXECUTE FUNCTION public.protect_profile_columns();
...
INSERT INTO public.site_settings (key, value) VALUES
  ...
  ('site_name', to_jsonb('Smart Deal'::text)),
